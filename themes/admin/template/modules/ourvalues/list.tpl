...
<div class="main-panel">
    <div class="sec-head">
        ...
        <div class="sec-head-btns">
            <a href="<?php echo $add; ?>" class="btn bts89067 btn890-890-890 opt"> + Add Value </a>
        </div>
    </div>
                                            ...
                                            <td>
                                                <div class="custom-control custom-switch">
                                                    <input type="checkbox" class="custom-control-input status-toggle"
                                                        id="statusToggle<?php echo $value['value_id']; ?>"
                                                        <?php echo ($value['status']) ? 'checked' : ''; ?>
                                                        data-status-id="<?php echo $value['value_id']; ?>">
                                                    <label class="custom-control-label"
                                                        for="statusToggle<?php echo $value['value_id']; ?>">
                                                        <?php echo ($value['status']) ? 'Active' : 'Inactive'; ?>
                                                    </label>
                                                </div>
                                            </td>
                                            <td class="text-right">
                                                <a style="display: inline-block;" href="<?php echo $value['edit']; ?>"
                                                    data-toggle="tooltip" title="<?php echo $button_edit; ?>"
                                                    class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                                                <form style="display: inline-block;"
                                                    action="<?php echo $value['delete']; ?>" method="post"
                                                    enctype="multipart/form-data"
                                                    id="del_value<?php echo $value['value_id']; ?>">
                                                    <input type="hidden" name="value_id"
                                                        value="<?php echo $value['value_id']; ?>">
                                                    <button type="button" data-toggle="tooltip"
                                                        title="<?php echo $button_delete; ?>" class="btn btn-danger"
                                                        onClick='submitDeleteForm("del_value<?php echo $value['value_id']; ?>")'><i
...
<script>
    $(document).on('change', 'input[type="checkbox"]', function() {
        ...
        var valueId = checkbox.data('status-id');
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    rowData[1] =
                        ...
                        valueId + '" ' + (newStatus ? 'checked' : '') + ' data-status-id="' +
                        valueId + '"> <label class="custom-control-label" for="statusToggle' +
                        valueId + '">' + newStatusText + '</label> </div>';
