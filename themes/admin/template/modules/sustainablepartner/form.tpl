                                ...
                                <div class="col-lg-2 col-md-2 ">
                                    <?php if ($logo) { ?>
                                     <button type="button" class="btn btn-danger btn-sm delete-image-btn"
                                        ...
                                        onclick="deleteSustainableImage('<?= $sustainablepartner_id ?>')">
...
<script>
function deleteSustainableImage(sustainablepartner_id) {
    if (confirm('Are you sure you want to delete this Logo?')) {
        fetch('<?php echo $deleteImage; ?>&sustainablepartner_id=' + sustainablepartner_id)
        .then(res => res.json())
        .then(data => {
            if (data.success) {
                const img = document.getElementById('blogo');
                img.src = '../uploads/image/no-image.png?' + new Date().getTime(); // prevent cache
                document.getElementById('hidden_logo').value = '';
                const btn = document.querySelector('.delete-image-btn');
                if (btn) btn.style.display = 'none';
                alert('Logo deleted successfully.');
            } else {
                alert(data.error || 'Error deleting Logo.');
            }
        })
        .catch(err => {
            console.error('Error deleting Logo image:', err);
            alert('Something went wrong while deleting the Logo.');
        });
    }
}
</script>