...
<div class="main-panel">
    <div class="sec-head">
        ...
        <div class="sec-head-btns">
            <a href="<?php echo $add; ?>" class="btn bts89067 btn890-890-890 opt"> + Add Sustainable Partner </a>
        </div>
    </div>
                    ...
                    <div class="card">
                        <div class="card-bodys">
                            ...
                            <table id="sharp-table" class="table table-striped table-bordered table-hover"
                                ...
                                <tbody>
                                    <?php
                                    foreach ($sustainablepartners as $sustainablepartner) { ?>
                                        <tr>
                                            <td>
                                                <?= $sustainablepartner['title'] ?>
                                            </td>
                                            <td>
                                                <div class="custom-control custom-switch">
                                                    <input type="checkbox" class="custom-control-input status-toggle"
                                                        id="statusToggle<?php echo $sustainablepartner['sustainablepartner_id']; ?>"
                                                        <?php echo ($sustainablepartner['status']) ? 'checked' : ''; ?>
                                                        data-status-id="<?php echo $sustainablepartner['sustainablepartner_id']; ?>">
                                                    <label class="custom-control-label"
                                                        for="statusToggle<?php echo $sustainablepartner['sustainablepartner_id']; ?>">
                                                        <?php echo ($sustainablepartner['status']) ? 'Active' : 'Inactive'; ?>
                                                    </label>
                                                </div>
                                            </td>
                                            <td class="text-right">
                                                <a style="display: inline-block;" href="<?php echo $sustainablepartner['edit']; ?>"
                                                    data-toggle="tooltip" title="<?php echo $button_edit; ?>"
                                                    class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                                                <form style="display: inline-block;"
                                                    action="<?php echo $sustainablepartner['delete']; ?>" method="post"
                                                    enctype="multipart/form-data"
                                                    id="del_sustainablepartner<?php echo $sustainablepartner['sustainablepartner_id']; ?>">
                                                    <input type="hidden" name="sustainablepartner_id"
                                                        value="<?php echo $sustainablepartner['sustainablepartner_id']; ?>">
                                                    <button type="button" data-toggle="tooltip"
                                                        title="<?php echo $button_delete; ?>" class="btn btn-danger"
                                                        onClick='submitDeleteForm("del_sustainablepartner<?php echo $sustainablepartner['sustainablepartner_id']; ?>")'><i
...
<script>
    $(document).on('change', 'input[type="checkbox"]', function() {
        ...
        var SustainablepartnerId = checkbox.data('status-id');
        var newStatus = checkbox.is(':checked') ? 1 : 0;
        var newStatusText = newStatus ? 'Active' : 'Inactive';
        console.log(SustainablepartnerId);
        statusLabel.text(newStatusText);
        $('#loader').show();
        let data = {
            sustainablepartner_id: SustainablepartnerId,
            status: newStatus
        };
        $.ajax({
            url: '<?php echo $ajaxsustainablepartnerstatus; ?>',
            method: 'POST',
            data,
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    rowData[1] =
                        ...
                        SustainablepartnerId + '" ' + (newStatus ? 'checked' : '') + ' data-status-id="' +
                        SustainablepartnerId + '"> <label class="custom-control-label" for="statusToggle' +
                        SustainablepartnerId + '">' + newStatusText + '</label> </div>';
