...
<div class="main-panel">
    <div class="sec-head">
        ...
        <div class="sec-head-btns">
            <a href="<?php echo $add; ?>" class="btn bts89067 btn890-890-890 opt"> + Add Attribute Value </a>
        </div>
    </div>
                                            ...
                                            <td>
                                                <div class="custom-control custom-switch">
                                                    <input type="checkbox" class="custom-control-input status-toggle"
                                                        id="statusToggle<?php echo $attribute_value['attribute_value_id']; ?>"
                                                        <?php echo ($attribute_value['status']) ? 'checked' : ''; ?>
                                                        data-status-id="<?php echo $attribute_value['attribute_value_id']; ?>">
                                                    <label class="custom-control-label"
                                                        for="statusToggle<?php echo $attribute_value['attribute_value_id']; ?>">
                                                        <?php echo ($attribute_value['status']) ? 'Active' : 'Inactive'; ?>
                                                    </label>
                                                </div>
                                            </td>
                                            <td class="text-right">
                                                <a style="display: inline-block;" href="<?php echo $attribute_value['edit']; ?>"
                                                    data-toggle="tooltip" title="Edit"
                                                    class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                                                <form style="display: inline-block;"
                                                    action="<?php echo $attribute_value['delete']; ?>" method="post"
                                                    enctype="multipart/form-data"
                                                    id="del_attribute_value<?php echo $attribute_value['attribute_value_id']; ?>">
                                                    <input type="hidden" name="attribute_value_id"
                                                        value="<?php echo $attribute_value['attribute_value_id']; ?>">
                                                    <button type="button" data-toggle="tooltip"
                                                        title="Delete" class="btn btn-danger"
                                                        onClick='submitDeleteForm("del_attribute_value<?php echo $attribute_value['attribute_value_id']; ?>")'><i
...
<script>
    $(document).on('change', 'input[type="checkbox"]', function() {
        ...
        var attributeValueId = checkbox.data('status-id');
        ...
        $.ajax({
            url: '<?php echo $ajaxattributevaluestatus; ?>',
            method: 'POST',
            data: data,
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    rowData[1] = '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input status-toggle" id="statusToggle' + attributeValueId + '" ' + (newStatus ? 'checked' : '') + ' data-status-id="' + attributeValueId + '"> <label class="custom-control-label" for="statusToggle' + attributeValueId + '">' + newStatusText + '</label> </div>';
