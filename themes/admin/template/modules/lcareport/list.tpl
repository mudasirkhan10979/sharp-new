...
<div class="main-panel">
    <div class="sec-head">
        ...
        <div class="sec-head-btns">
            <a href="<?php echo $add; ?>" class="btn bts89067 btn890-890-890 opt"> + Add LCA Report </a>
        </div>
    </div>
                    ...
                    <div class="card">
                        <div class="card-bodys">
                            ...
                            <table id="sharp-table" class="table table-striped table-bordered table-hover"
                                ...
                                <tbody>
                                    <?php
                                    foreach ($lcareports as $lcareport) { ?>
                                        <tr>
                                            <td>
                                                <?= $lcareport['title'] ?>
                                            </td>
                                            <td>
                                                <div class="custom-control custom-switch">
                                                    <input type="checkbox" class="custom-control-input status-toggle"
                                                        id="statusToggle<?php echo $lcareport['lcareport_id']; ?>"
                                                        <?php echo ($lcareport['status']) ? 'checked' : ''; ?>
                                                        data-status-id="<?php echo $lcareport['lcareport_id']; ?>">
                                                    <label class="custom-control-label"
                                                        for="statusToggle<?php echo $lcareport['lcareport_id']; ?>">
                                                        <?php echo ($lcareport['status']) ? 'Active' : 'Inactive'; ?>
                                                    </label>
                                                </div>
                                            </td>
                                            <td class="text-right">
                                                <a style="display: inline-block;" href="<?php echo $lcareport['edit']; ?>"
                                                    data-toggle="tooltip" title="<?php echo $button_edit; ?>"
                                                    class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                                                <form style="display: inline-block;"
                                                    action="<?php echo $lcareport['delete']; ?>" method="post"
                                                    enctype="multipart/form-data"
                                                    id="del_lcareport<?php echo $lcareport['lcareport_id']; ?>">
                                                    <input type="hidden" name="lcareport_id"
                                                        value="<?php echo $lcareport['lcareport_id']; ?>">
                                                    <button type="button" data-toggle="tooltip"
                                                        title="<?php echo $button_delete; ?>" class="btn btn-danger"
                                                        onClick='submitDeleteForm("del_lcareport<?php echo $lcareport['lcareport_id']; ?>")'><i
...
<script>
    $(document).on('change', 'input[type="checkbox"]', function() {
        ...
        var lcareportId = checkbox.data('status-id');
        var newStatus = checkbox.is(':checked') ? 1 : 0;
        var newStatusText = newStatus ? 'Active' : 'Inactive';
        console.log(lcareportId);
        statusLabel.text(newStatusText);
        $('#loader').show();
        let data = {
            lcareport_id: lcareportId,
            status: newStatus
        };
        $.ajax({
            url: '<?php echo $ajaxlcareportstatus; ?>',
            method: 'POST',
            data,
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    rowData[1] =
                        ...
                        lcareportId + '" ' + (newStatus ? 'checked' : '') + ' data-status-id="' +
                        lcareportId + '"> <label class="custom-control-label" for="statusToggle' +
                        lcareportId + '">' + newStatusText + '</label> </div>';
