...
<div class="main-panel">
    <div class="sec-head">
        ...
        <div class="sec-head-btns">
            <a href="<?php echo $add; ?>" class="btn bts89067 btn890-890-890 opt"> + Add Source Code Download </a>
        </div>
    </div>
                                            ...
                                            <td>
                                                <div class="custom-control custom-switch">
                                                    <input type="checkbox" class="custom-control-input status-toggle"
                                                        id="statusToggle<?php echo $source['source_id']; ?>"
                                                        <?php echo ($source['status']) ? 'checked' : ''; ?>
                                                        data-status-id="<?php echo $source['source_id']; ?>">
                                                    <label class="custom-control-label"
                                                        for="statusToggle<?php echo $source['source_id']; ?>">
                                                        <?php echo ($source['status']) ? 'Active' : 'Inactive'; ?>
                                                    </label>
                                                </div>
                                            </td>
                                            <td class="text-right">
                                                <a style="display: inline-block;" href="<?php echo $source['edit']; ?>"
                                                    data-toggle="tooltip" title="<?php echo $button_edit; ?>"
                                                    class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                                                <form style="display: inline-block;"
                                                    action="<?php echo $source['delete']; ?>" method="post"
                                                    enctype="multipart/form-data"
                                                    id="del_source<?php echo $source['source_id']; ?>">
                                                    <input type="hidden" name="source_id"
                                                        value="<?php echo $source['source_id']; ?>">
                                                    <button type="button" data-toggle="tooltip"
                                                        title="<?php echo $button_delete; ?>" class="btn btn-danger"
                                                        onClick='submitDeleteForm("del_source<?php echo $source['source_id']; ?>")'><i
...
<script>
    $(document).on('change', 'input[type="checkbox"]', function() {
        ...
        var SourceId = checkbox.data('status-id');
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    rowData[1] =
                        ...
                        SourceId + '" ' + (newStatus ? 'checked' : '') + ' data-status-id="' +
                        SourceId + '"> <label class="custom-control-label" for="statusToggle' +
                        SourceId + '">' + newStatusText + '</label> </div>';
