...
<div class="main-panel">
    <div class="sec-head">
        ...
        <div class="sec-head-btns">
            <a href="<?php echo $add; ?>" class="btn bts89067 btn890-890-890 opt"> + Add Attribute </a>
        </div>
    </div>
                            ...
                            <table id="sharp-table" class="table table-striped table-bordered table-hover"
                                ...
                                <thead>
                                    ...
                                    <tr class="stdfilters">
                                        ...
                                        <!-- <th><input type="text" placeholder="Category ID"></th> -->
                                        <!-- <th><input type="text" placeholder="Sort Order"></th> -->
                                        <th id="drop-searc"></th>
                                        <!-- <th><input type="text" placeholder="Added Date"></th> -->
                                        <th></th>
                                    </tr>
                                </thead>
                                <tbody>
                                            ...
                                            <td>
                                                <div class="custom-control custom-switch">
                                                    <input type="checkbox" class="custom-control-input status-toggle"
                                                        id="statusToggle<?php echo $attribute['attribute_id']; ?>"
                                                        <?php echo ($attribute['status']) ? 'checked' : ''; ?>
                                                        data-status-id="<?php echo $attribute['attribute_id']; ?>">
                                                    <label class="custom-control-label"
                                                        for="statusToggle<?php echo $attribute['attribute_id']; ?>">
                                                        <?php echo ($attribute['status']) ? 'Active' : 'Inactive'; ?>
                                                    </label>
                                                </div>
                                            </td>
                                            <!-- <td><?= date('Y-m-d', strtotime($attribute['added_date'])) ?></td> -->
                                            <td class="text-right">
                                                <a style="display: inline-block;" href="<?php echo $attribute['edit']; ?>"
                                                    data-toggle="tooltip" title="Edit"
                                                    class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                                                <form style="display: inline-block;"
                                                    action="<?php echo $attribute['delete']; ?>" method="post"
                                                    enctype="multipart/form-data"
                                                    id="del_attribute<?php echo $attribute['attribute_id']; ?>">
                                                    <input type="hidden" name="attribute_id"
                                                        value="<?php echo $attribute['attribute_id']; ?>">
                                                    <button type="button" data-toggle="tooltip"
                                                        title="Delete" class="btn btn-danger"
                                                        onClick='submitDeleteForm("del_attribute<?php echo $attribute['attribute_id']; ?>")'><i
...
<script>
    $(document).on('change', 'input[type="checkbox"]', function() {
        ...
        var attributeId = checkbox.data('status-id');
        ...
        $.ajax({
            url: '<?php echo $ajaxattributestatus; ?>',
            method: 'POST',
            data: data,
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    rowData[1] = '<div class="custom-control custom-switch"><input type="checkbox" class="custom-control-input status-toggle" id="statusToggle' + attributeId + '" ' + (newStatus ? 'checked' : '') + ' data-status-id="' + attributeId + '"> <label class="custom-control-label" for="statusToggle' + attributeId + '">' + newStatusText + '</label> </div>';
                    row.data(rowData).draw(false);
                } else {
                    checkbox.prop('checked', !checkbox.is(':checked'));
                    statusLabel.text(checkbox.is(':checked') ? 'Active' : 'In-active');
                }
            },
