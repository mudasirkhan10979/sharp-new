...
<div class="main-panel">
    <div class="sec-head">
        ...
        <div class="sec-head-btns">
            <a href="<?php echo $add; ?>" class="btn bts89067 btn890-890-890 opt"> + Add Our Location </a>
        </div>
    </div>
                                    ...
                                    <tr> 
                                        ...
                                        <div class="custom-control custom-switch">
                                                <input type="checkbox" class="custom-control-input status-toggle"
                                                    id="statusToggle<?php echo $location['location_id']; ?>"
                                                    <?php echo ($location['publish']) ? 'checked' : ''; ?>
                                                    data-status-id="<?php echo $location['location_id']; ?>">
                                                <label class="custom-control-label"
                                                    for="statusToggle<?php echo $location['location_id']; ?>">
                                                    <?php echo ($location['publish']) ? 'Active' : 'Inactive'; ?>
                                                </label>
                                            </div>
                                        </td>
                                        <td class="text-right">
                                            <a style="display: inline-block;" href="<?php echo $location['edit']; ?>"
                                                data-toggle="tooltip" title="<?php echo $button_edit; ?>"
                                                class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                                            <form style="display: inline-block;"
                                                action="<?php echo $location['delete']; ?>" method="post"
                                                enctype="multipart/form-data"
                                                id="del_location<?php echo $location['location_id']; ?>">
                                                <input type="hidden" name="location_id"
                                                    value="<?php echo $location['location_id']; ?>">
                                                <button type="button" data-toggle="tooltip"
                                                    title="<?php echo $button_delete; ?>" class="btn btn-danger"
                                                    onClick='submitDeleteForm("del_location<?php echo $location['location_id']; ?>")'><i
...
$(document).on('change', 'input[type="checkbox"]', function() {
    ...
    var locationId = checkbox.data('status-id');
    ...
    $.ajax({
        url: '<?php echo $ajaxdourlocationstatus; ?>',
        method: 'POST',
        data,
            ...
            if (response.success) {
                if (response.success) {
                    ...
                    rowData[1] =
                        ...
                        locationId + '" ' + (newStatus ? 'checked' : '') + ' data-status-id="' + locationId +
                        '"> <label class="custom-control-label" for="statusToggle' + locationId + '">' +
