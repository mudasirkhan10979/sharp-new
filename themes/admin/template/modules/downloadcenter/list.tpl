...
<div class="main-panel">
    <div class="sec-head">
        <div class="sec-head-title">
            <h3>Download Centers</h3>
        </div>
        <div class="sec-head-btns">
            <a href="<?php echo $add; ?>" class="btn bts89067 btn890-890-890 opt"> + Add Download Center </a>
        </div>
    </div>
                                            ...
                                            <td>
                                                <div class="custom-control custom-switch">
                                                    <input type="checkbox" class="custom-control-input status-toggle"
                                                        id="statusToggle<?php echo $download['download_id']; ?>"
                                                        <?php echo ($download['status']) ? 'checked' : ''; ?>
                                                        data-status-id="<?php echo $download['download_id']; ?>">
                                                    <label class="custom-control-label"
                                                        for="statusToggle<?php echo $download['download_id']; ?>">
                                                        <?php echo ($download['status']) ? 'Active' : 'Inactive'; ?>
                                                    </label>
                                                </div>
                                            </td>
                                            <td class="text-right">
                                                <a style="display: inline-block;" href="<?php echo $download['edit']; ?>"
                                                    data-toggle="tooltip" title="<?php echo $button_edit; ?>"
                                                    class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                                                <form style="display: inline-block;"
                                                    action="<?php echo $download['delete']; ?>" method="post"
                                                    enctype="multipart/form-data"
                                                    id="del_download<?php echo $download['download_id']; ?>">
                                                    ...
                                                    <button type="button" data-toggle="tooltip"
                                                        ...
                                                        onClick='submitDeleteForm("del_download<?php echo $download['download_id']; ?>")'><i
...
<script>
    $(document).on('change', 'input[type="checkbox"]', function() {
        ...
        var DownloadId = checkbox.data('status-id');
        ...
        $.ajax({
            url: '<?php echo $ajaxdownloadstatus; ?>',
            method: 'POST',
            data,
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    rowData[1] =
                        ...
                        DownloadId + '" ' + (newStatus ? 'checked' : '') + ' data-status-id="' +
                        DownloadId + '"> <label class="custom-control-label" for="statusToggle' +
                        DownloadId + '">' + newStatusText + '</label> </div>';
