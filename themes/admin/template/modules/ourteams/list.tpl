...
<div class="main-panel">
    <div class="sec-head">
        ...
        <div class="sec-head-btns">
            <a href="<?php echo $add; ?>" class="btn bts89067 btn890-890-890 opt"> + Add Our Team </a>
        </div>
    </div>
                                            ...
                                            <td>
                                                <div class="custom-control custom-switch">
                                                    <input type="checkbox" class="custom-control-input status-toggle"
                                                        id="statusToggle<?php echo $team['team_id']; ?>"
                                                        <?php echo ($team['status']) ? 'checked' : ''; ?>
                                                        data-status-id="<?php echo $team['team_id']; ?>">
                                                    <label class="custom-control-label"
                                                        for="statusToggle<?php echo $team['team_id']; ?>">
                                                        <?php echo ($team['status']) ? 'Active' : 'Inactive'; ?>
                                                    </label>
                                                </div>
                                            </td>
                                            <td class="text-right">
                                                <a style="display: inline-block;" href="<?php echo $team['edit']; ?>"
                                                    data-toggle="tooltip" title="<?php echo $button_edit; ?>"
                                                    class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                                                <form style="display: inline-block;"
                                                    action="<?php echo $team['delete']; ?>" method="post"
                                                    enctype="multipart/form-data"
                                                    id="del_team<?php echo $team['team_id']; ?>">
                                                    <input type="hidden" name="team_id"
                                                        value="<?php echo $team['team_id']; ?>">
                                                    <button type="button" data-toggle="tooltip"
                                                        title="<?php echo $button_delete; ?>" class="btn btn-danger"
                                                        onClick='submitDeleteForm("del_team<?php echo $team['team_id']; ?>")'><i
...
<script>
    $(document).on('change', 'input[type="checkbox"]', function() {
        ...
        var teamId = checkbox.data('status-id');
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    rowData[1] =
                        ...
                        teamId + '" ' + (newStatus ? 'checked' : '') + ' data-status-id="' +
                        teamId + '"> <label class="custom-control-label" for="statusToggle' +
                        teamId + '">' + newStatusText + '</label> </div>';
