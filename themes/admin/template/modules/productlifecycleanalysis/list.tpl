...
<div class="main-panel">
    <div class="sec-head">
        <div class="sec-head-title">
            <h3>Product Lifecycle Analysis</h3>
        </div>
        <div class="sec-head-btns">
            <a href="<?php echo $add; ?>" class="btn bts89067 btn890-890-890 opt"> + Add Product Lifecycle Analysis </a>
        </div>
    </div>
                    ...
                    <div class="card">
                        <div class="card-bodys">
                            ...
                            <table id="sharp-table" class="table table-striped table-bordered table-hover"
                                ...
                                <tbody>
                                    <?php
                                    foreach ($productlifecycleanalysis as $plifecycleanalysis) { ?>
                                        <tr>
                                            <td>
                                                <?= $plifecycleanalysis['title'] ?>
                                            </td>
                                            <td>
                                                <div class="custom-control custom-switch">
                                                    <input type="checkbox" class="custom-control-input status-toggle"
                                                        id="statusToggle<?php echo $plifecycleanalysis['productlife__id']; ?>"
                                                        <?php echo ($plifecycleanalysis['status']) ? 'checked' : ''; ?>
                                                        data-status-id="<?php echo $plifecycleanalysis['productlife__id']; ?>">
                                                    <label class="custom-control-label"
                                                        for="statusToggle<?php echo $plifecycleanalysis['productlife__id']; ?>">
                                                        <?php echo ($plifecycleanalysis['status']) ? 'Active' : 'Inactive'; ?>
                                                    </label>
                                                </div>
                                            </td>
                                            <td class="text-right">
                                                <a style="display: inline-block;" href="<?php echo $plifecycleanalysis['edit']; ?>"
                                                    data-toggle="tooltip" title="<?php echo $button_edit; ?>"
                                                    class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                                                <form style="display: inline-block;"
                                                    action="<?php echo $plifecycleanalysis['delete']; ?>" method="post"
                                                    enctype="multipart/form-data"
                                                    id="del_plifecycleanalysis<?php echo $plifecycleanalysis['productlife__id']; ?>">
                                                    <input type="hidden" name="productlife__id"
                                                        value="<?php echo $plifecycleanalysis['productlife__id']; ?>">
                                                    <button type="button" data-toggle="tooltip"
                                                        title="<?php echo $button_delete; ?>" class="btn btn-danger"
                                                        onClick='submitDeleteForm("del_plifecycleanalysis<?php echo $plifecycleanalysis['productlife__id']; ?>")'><i
...
<script>
    $(document).on('change', 'input[type="checkbox"]', function() {
        ...
        var productlifeId = checkbox.data('status-id');
        var newStatus = checkbox.is(':checked') ? 1 : 0;
        var newStatusText = newStatus ? 'Active' : 'Inactive';
        console.log(productlifeId);
        statusLabel.text(newStatusText);
        $('#loader').show();
        let data = {
            productlife__id: productlifeId,
            status: newStatus
        };
        $.ajax({
            url: '<?php echo $ajaxproductlifecycleanalysisstatus; ?>',
            method: 'POST',
            data,
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    rowData[1] =
                        ...
                        productlifeId + '" ' + (newStatus ? 'checked' : '') + ' data-status-id="' +
                        productlifeId + '"> <label class="custom-control-label" for="statusToggle' +
                        productlifeId + '">' + newStatusText + '</label> </div>';
