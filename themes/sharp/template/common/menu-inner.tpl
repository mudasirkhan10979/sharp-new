          ...
          <div class="main-menu desktop">
            <div class="main-menu-inn">
              <div class="main-nav-left">
                <?php
                // Inject dynamic menu sections inside Business Solutions
                if (!empty($categories) && !empty($documentMenus)) {
                  foreach ($categories as &$parent) {
                    // Find Business Solutions category (category_id 34)
                    if ($parent['category_id'] == 34 && !empty($parent['children'])) {
                      $newChildren = [];
                      foreach ($parent['children'] as $child) {
                        $newChildren[] = $child;
                        // After Calculators category (category_id 9)
                        if ($child['category_id'] == 9) {
                          ...
                          foreach ($documentMenus as $menuItem) {
                            $dynamicChildren[] = array(
                              ...
                              'image'              => '',
                              'seo_url'            => $menuItem['url'], // Use the URL directly from documentMenus
                              'children'           => array()
                            );
                          }
                          // Add the main Document & Printing Solutions parent item
                          $newChildren[] = array(
                            'category_id'        => 'doc_print_solutions',
                            'title'              => 'Document & Printing Solutions',
                            'short_description'  => 'Document & Printing Solutions',
                            'image'              => '',
                            'seo_url'            =>  'intelligent-print', // leave empty to make it non-clickable
                      ...
                      break; // stop after finding the Business Solutions category
                    }
                }
                unset($parent); // unset reference
                }
                // Inject Plasmacluster Technology after category_id 34
                if (!empty($categories) && !empty($plasmamenu)) {
                  $merged = array();
                ...
                <span class="search" id="headerSearch">
                  ...
                  <div class="search-popup" id="searchPopup">
                    <div class="search-popup-inn">
                      <input type="text" placeholder="<?php echo $text_placeholder; ?>" id="searchInput">
                      <button type="button" id="closeSearch">&times;</button>
                    </div>
                    <div id="searchResultsPopup" style="display:none;"></div>
                  </div>
                </span>
  ...
  (function() {
    ...
    document.addEventListener("click", (e) => {
      if (!searchPopup.contains(e.target) && e.target !== searchIcon) {
        searchPopup.classList.remove("show");
        searchInput.value = ""; //  Clear input when clicking outside
        clearResults();
      }
    });
    // Debounce helper
