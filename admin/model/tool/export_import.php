...
class ModelToolExportImport extends Model {
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		...
		$sql .= "`tax_class_id`,`viewed`,`length`,`width`,`height`,`length_class_id`,`sort_order`,`subtract`,`minimum`,`color`,`size`) VALUES ";
		...
		$sql .= "$tax_class_id,$viewed,$length,$width,$height,'$length_class_id','$sort_order','$subtract','$minimum','$color','$size');";
		...
		if ($keyword) {
			...
			$this->db->query($sql);
				...
				$sql = "INSERT INTO `".DB_PREFIX."product_to_store` (`product_id`,`store_id`) VALUES ($product_id,$store_ids);";
				$this->db->query($sql);
			
		
		
		$sql = "INSERT INTO `".DB_PREFIX."product_to_layout` (`product_id`,`store_id`,`layout_id`) VALUES ($product_id,$store_ids,$layout);";
			$this->db->query($sql);
	...
	protected function storeProductOptionIntoDatabase( &$product_option, &$old_product_option_ids ) {
		...
		$sql_productname  = "SELECT product_id FROM `".DB_PREFIX."product_description` WHERE name='".$this->db->escape($product_id)."'";
		$query_productname = $this->db->query( $sql_productname );
		
	    $product_id=$query_productname->row['product_id'];
	...
	protected function uploadProductOptionValues( &$reader, $incremental, &$available_product_ids ) {
	    ...
	    $product_id_new=$query_productname->row['product_id'];
		
			
				$product_option_ids = $this->getProductOptionIds( $product_id_new );
