<?php
class ModelSeoUrl extends Model {
	...
	public function getSeoUrlsByKeyword($keyword) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "aliases` WHERE url = '" . $this->db->escape($keyword) . "'");
...
public function chkUUrl($string)
	{
		
		$sql = "SELECT url FROM  aliases WHERE url='" . $this->db->escape($string) . "'";
		$rs = $this->db->query($sql);
		return $rs->row['url'];
	}
public function checkUrlById($id){
		$sql = "SELECT url FROM  aliases WHERE slog_id='" . (int)$id . "'";
		$rs = $this->db->query($sql);
		return $rs->row['url'];
}
